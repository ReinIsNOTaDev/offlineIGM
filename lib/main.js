...
// using ajax now as a catch-all function querying either locally or through proxy
async function ajax(url,callback) {
	var check = await checkServer();
	console.log("check is "+check);
	if (check==true) {
		serverFetch(url,callback);
		console.log("serverfetch");
	} else {
		localFetch(url,callback);
		console.log("localfetch");
	}
	console.log("ajax called");
}

// function to check if the proxy server exists
...
// offlineIGM: fetch using browser fetch api
async function localFetch(url,callback)
{
	try {
		const fetchRequest = await fetch(url);
		const serverResponse = await fetchRequest.text();
		if (callback) {
		callback(serverResponse);
		}
	} catch (error) {
		G.parseError(error.message);
	}
}

// offlineIGM: fetch using a proxy server
async function serverFetch(url,callback)
{
	try {
		const fetchRequest = await fetch("http://127.0.0.1:3000/?u="+url);
		const serverResponse = await fetchRequest.text();
		if (callback) {
			callback(serverResponse);
		}
